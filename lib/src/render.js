...
function getFileTableRL(project, minCoverage, emoji) {
    ...
    for (const module of project.modules) {
        for (let index = 0; index < module.files.length; index++) {
            ...
            renderRow(moduleName, `[${file.name}](${file.url})`, file.overall.percentage, deltaCoverage, file.changed?.percentage ?? null, project.isMultiModule);
        }
    }
...
function getDeltaCoverage(changed) {
    if (!changed)
        return null;
